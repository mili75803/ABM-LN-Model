...
public class TCell extends Agent
{
    ...
    public TCell(SimState state, LymphNodes LN, double LNside, double scale, Network DCList, int noOfTCPerCloneType, Semaphore sem, int[] TotalTCCompleted,
             int onlyInitialTime, int fileNum, int[] count, int[] DCFirstDetectTime, long[] sumContactTime, long[] lastFirstContactTime, long[] contactTime,  ContinuousPortrayal3D agitatedYardPortrayal, Display3D display3d)
    {
        this.state = state;
        ...
        this.onlyInitialTime = onlyInitialTime;
...
//        new_vect.setCoordinates(rotateAboutVector(new_vect, previous_vector, change_in_azmuth));
    ...
    public void remove(SimState state){
        AgentsSetting_ as = (AgentsSetting_)state;
...
public void step(SimState state)
    {
        boolean flag = true;
        AgentsSetting_ students = (AgentsSetting_) state;
                    ...
                    if ((x_pos >= LN_x_left_corner && x_pos <= LN_x_right_corner) && (y_pos >= LN_y_bottom_corner && y_pos <= LN_y_top_corner) && (z_pos >= z_back_LN_corner && z_pos <= z_front_LN_corner)) 
                    {
                        ...
                        for (int i = 0; i < myDC.numObjs; i++) {
                            ...
                            if(dis < 5.00){
                                DCdetected = true;   
                                /*{
                                if(DCFirstDetectTime[this.groupLocalID] == 0){
                                    DCFirstDetectTime[this.groupLocalID] = tIteration + 1;
                                    String FirstContactTime = DCFirstDetectTime[this.groupLocalID] + "";
                                    fileWriteFirstContact(FirstContactTime);    //everytime a TC detects the first DC 
                                }
                                myDC.remove(i);
                                */
                                this.agitatedYardPortrayal.setPortrayalForObject(this, new ConePortrayal3D(Color.red, 2.0));
                                ...
                                //if 25% TC contacted with DC, we take the average
                                //long checkTcell = Math.round(0.25 * TotalTC);
